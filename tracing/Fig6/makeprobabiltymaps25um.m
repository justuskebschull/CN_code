...
%% let's define areas where B~A, and give these to Nomi to calculate secondary projections.
% cutofflo=0.3;
% cutoffhi=0.5;
% Bdiff=B-A;
% Bspecificlo=Bdiff<cutofflo & B>cutofflo;
% 
% Bspecificlo_scaled=uint16(Bspecificlo.*~mask*2^16);
% 
% mkdir 'overlap'
% 
% for i=1:size(A,3)
%     imwrite(squeeze(Bspecificlo_scaled(:,:,i)),['overlap/classBscificlo',int2str(i),'.tif']);
% end
